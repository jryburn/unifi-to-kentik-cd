#!/usr/bin/env python
"""Update Kentik custom dimension populators from UniFi client data."""
#
#   update_custom_dimension.py  written by Justin Ryburn (jryburn@kentik.com)
#   2022 Apr 1
#
#   This simple python script grabs a client list from a Ubiquiti Unifi
#   controller and saves them to a .csv file.
#
#   It then uses the Kentik bulk API to quickly load custom dimension populator
#   data.
#
#   This script uses the Kentik bulk API written by Blake Caldwell which can
#   be located at:
...
#
# "POPULATOR_NAME" and "POPULATOR_DATA" should be the column headers for the
# CSV file.
#
POPULATOR_NAME = "hostname"
POPULATOR_DATA = "mac_addr"
# This script automatically creates both the source and destination populators
# for a given custom dimension.
# In the Kentik portal, you need to create two custom dimensions. The database
# fields must be in the format of
# "c_dst_dimension" and "c_src_dimension". The script will add the populators
# and MAC address to both the source and destination dimensions.
...
def pull_clients():
    """Pull client records from the UniFi controller and write them to CSV."""
...
def push_to_kentik():
    """Read the CSV and submit custom dimension populators to Kentik."""
    # initialize a batch that will replace all populators
    batch_src = tagging.Batch(True)
    batch_dst = tagging.Batch(True)

    # Use the CSV reader library to read in the CSV file and send the data to
    # the batch API. The column headings should be labeled mac_addr and
    # hostname
    ...
    # two batches (for src and dst) so we need two criteria, addresses and
    # populators)
    print("Building batch for upload...")
    for item in tags.items():
        populator = item[0]
        mac = item[1][0]
        print(f"Adding populator {populator} with mac {mac}")
    ...
    # -----
    # Showtime! Submit the batch as populators for the configured custom
    # dimension - library will take care of chunking the requests into smaller
    # HTTP payloads
    # -----
    ...
    #  Wait and display the dst results
    for _ in range(1, 12):
        time.sleep(5)
    ...
    #  Wait and display the src results
    for _ in range(1, 12):
        time.sleep(5)
